...
func (h *Handlers) QueryGameserverPartial(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "id")
	gameserver, ok := h.getGameserver(w, id)
	if !ok {
		return
	...
	if gameserver.Status != models.StatusRunning {
		...
		if err := h.tmpl.ExecuteTemplate(w, "gameserver-query.html", data); err != nil {
			HandleError(w, InternalError(err, "Failed to render query"), "query")
		}
		return
	}
	...
	// Return the server info as HTML partial
	data := map[string]interface{}{
