...
func (dm *DatabaseManager) UpdateScheduledTask(task *models.ScheduledTask) error {
	...
	if result.Error != nil {
		return &databaseError{Op: "update_task", Msg: "failed to update scheduled task", Err: result.Error}
	}
...
func (dm *DatabaseManager) DeleteScheduledTask(id string) error {
	...
	if result.Error != nil {
		return &databaseError{Op: "delete_task", Msg: "failed to delete scheduled task", Err: result.Error}
	}
	if result.RowsAffected == 0 {
